...
Microsoft Store publication for version `0.5.0` is currently pending. Once certification is complete, the Store listing will be linked here as the only supported production installer.
...
Microsoft Store product `9NFCPJXQG9FG` is the only production and update channel starting with version `0.3.0`. Publication of version `0.5.0` is currently awaiting Store certification. The GitHub `v0.5.0` release records the source release and does not contain an unsigned installer.
